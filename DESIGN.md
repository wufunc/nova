# Nova Project Memory Management System Design
...
Each project can have a "constitution" (`.nova/constitution.md`) that defines the project's highest-priority principles and constraints. All operations of the memory system — whether the Agent's code modifications or memory writing — must not violate the constitution.
...
│ 3. Enter project → read .nova/constitution.md    │
└────────────────────┬──────────────────────────────────┘
                     │ triggers
┌────────────────────▼──────────────────────────────────┐
...
User sends first message
  ...
  → Agent reads .nova/constitution.md first (if exists)
  → /memory recall executes
     ├─ Check if .nova/memory/ exists
...
2. First session's `/memory recall` automatically creates the `.nova/memory/` directory structure
...
The only optional manual step is writing `.nova/constitution.md` to define the project's constitutional principles.
