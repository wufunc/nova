<!-- nova:start -->

# Development Workflow

## Memory Management

- At the start of every session, MUST use the Skill tool to invoke the **memory** skill with argument **recall** before processing any task.
- During development, proactively invoke the memory skill (recall mode) when encountering uncertain technical problems or when prior experience might be relevant.

## Constitution Protocol

- You MUST read `.nova/constitution.md` in the current repository first to understand the project's highest constitutional principles before proposing or implementing any solution.
- If `.nova/constitution.md` does not exist, proceed with best effort.
- If a proposed solution conflicts with the constitution, MUST explain the conflict and rationale first, then continue implementation only after that explanation.

<!-- nova:end -->
